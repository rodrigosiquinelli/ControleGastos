    ...
    <div className="max-w-4xl mx-auto space-y-6 text-left animate-in fade-in duration-500">
      ...
      <div className="bg-white p-10 rounded-[2.5rem] shadow-xl shadow-gray-200/50 border border-gray-100">
        <div className="flex flex-col md:flex-row justify-between items-center gap-8">
          <div>
            <h1 className="text-4xl font-black text-gray-900 uppercase tracking-tighter">{pessoa.nome}</h1>
            <p className="text-gray-400 font-bold mt-1 uppercase text-xs">Idade: {pessoa.idade} anos</p>
          </div>
          
          <div className="card-detalhes p-6 rounded-[2rem] flex gap-8 border border-gray-100">
            <div className="text-center">
              <p className="text-[10px] font-black text-gray-400 uppercase">Receitas</p>
              <p className="text-xl font-black text-green-600">R$ {totalReceita.toFixed(2)}</p>
            </div>
            <div className="text-center border-x px-8">
              <p className="text-[10px] font-black text-gray-400 uppercase">Despesas</p>
            ...
            {currentItems.map(t => (
              <tr key={t.id} className="hover:bg-gray-50/50 transition-colors">
                <td className="px-8 py-6">
                  ...
                  <span className={`text-[10px] font-black uppercase ${t.tipo === 2 ? 'text-green-500' : 'text-red-500'}`}>
