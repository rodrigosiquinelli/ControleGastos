...
export function Transacoes() {
  ...
  async function salvar(e: React.FormEvent) {
    ...
    if (c) {
      ...
      if (finalidade === "Receita" && tipoNumerico === 1) {
        return alert(
          `🚫 A categoria "${c.descricao}" é exclusiva para RECEITAS.`,
        );
      }
      if (finalidade === "Despesa" && tipoNumerico === 2) {
        return alert(
          `🚫 A categoria "${c.descricao}" é exclusiva para DESPESAS.`,
        );
      }
    }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500 text-left">
      ...
      >
        {/* Grid de Inputs de Texto e Número */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col gap-2">
            <label htmlFor="valor" className="text-sm font-bold text-gray-700">
              Valor
            </label>
            <input
              id="valor"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="flex flex-col gap-2">
            ...
            <select
              id="tipo"
              className="border-2 border-gray-100 rounded-2xl p-4 bg-gray-50/50"
              value={tipo}
          ...
          <div className="flex flex-col gap-2">
            <label htmlFor="pessoa" className="text-sm font-bold text-gray-700">
              Responsável
            </label>
            <select
              id="pessoa"
          ...
          <tbody className="divide-y divide-gray-50">
            {currentItems.map((t) => (
              <tr key={t.id} className="hover:bg-blue-50/20 transition-colors">
                <td className="px-8 py-6">
                  <div className="flex flex-col">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span
                        className={`text-[10px] font-black uppercase px-2 py-0.5 rounded ${t.tipo === 2 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                      >
                ...
                <td
                  className={`px-8 py-6 text-right font-black text-lg ${t.tipo === 2 ? "text-green-600" : "text-red-600"}`}
                >
