...
export function Categorias() {
  ...
  async function salvar(e: React.FormEvent) {
    ...
    const finalidadeNumerica = 
      finalidade === 'Receita' ? 2 : 
      finalidade === 'Ambas' ? 3 : 1;

    try {
      const payload = { descricao, finalidade: finalidadeNumerica };

      if (editandoId) {
        await api.put(`/Categorias/${editandoId}`, { ...payload, id: editandoId });
      ...
      const apiErro = error.response?.data?.Detailed || error.response?.data?.Message || "Erro ao salvar.";
      alert(`❌ ${apiErro}`);
    }
  }
    ...
    <div className="max-w-5xl mx-auto p-4 space-y-8 animate-in fade-in duration-500 text-left">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-l-4 border-green-500 pl-4">
        <div>
          ...
          <p className="text-gray-500">Defina os tipos de receitas e despesas</p>
        </div>
      ...
      <form onSubmit={salvar} className={`bg-white p-8 rounded-3xl shadow-xl border-2 transition-all ${editandoId ? 'border-green-500' : 'border-transparent'} grid grid-cols-1 md:grid-cols-3 gap-6 items-end`}>
        ...
        <div className="flex flex-col gap-2 text-left">
          <label className="text-sm font-bold text-gray-700 ml-1">Finalidade</label>
          <select 
        ...
        <div className="flex gap-2">
          ...
          <button className={`${editandoId ? 'bg-orange-500 hover:bg-orange-600' : 'bg-green-600 hover:bg-green-700'} text-white font-black py-5 px-8 rounded-2xl transition-all active:scale-95 flex-1`}>
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className={`w-2 h-2 rounded-full ${
                        fin === 2 ? 'bg-green-500' : fin === 1 ? 'bg-red-500' : 'bg-blue-500'
                      }`}></span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-gray-200 group-hover:text-green-500 group-hover:translate-x-1 transition-all text-xl">➔</span>
                  </div>
                </div>
              );
            })}
