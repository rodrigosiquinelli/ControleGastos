...
export function ListaPessoasRelatorio() {
  ...
  const calcularTotaisPessoa = (pessoaId: string) => {
    ...
    const receitas = tPessoa
      .filter((t) => t.tipo === 2)
      .reduce((acc, t) => acc + Number(t.valor), 0);
    const despesas = tPessoa
      .filter((t) => t.tipo === 1)
      .reduce((acc, t) => acc + Number(t.valor), 0);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 text-left animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button
            ...
            className="bg-white p-3 rounded-xl shadow-sm hover:bg-gray-50 font-bold border border-gray-100"
          >
            ←
          </button>
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  <p className="font-black text-gray-800 uppercase tracking-tight group-hover:text-blue-600 transition-colors">
                    {p.nome}
                  </p>
                ...
                <span className="text-blue-500 font-black text-xs opacity-0 group-hover:opacity-100 transition-all">
                  DETALHES →
                </span>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 pt-2 border-t border-gray-50">
                ...
                <div className="text-center">
                  ...
                  <p
                    className={`text-xs font-black ${saldo >= 0 ? "text-blue-600" : "text-red-600"}`}
                  >
                    R$ {saldo.toFixed(2)}
                  </p>
                </div>
      ...
      <div className="mt-12 bg-gray-900 p-8 rounded-[2.5rem] shadow-2xl shadow-blue-900/20 text-white relative overflow-hidden">
        <div className="absolute top-0 right-0 p-8 opacity-10 text-9xl font-black">
          Σ
        </div>
        <h2 className="text-xs font-black uppercase tracking-[0.3em] mb-6 text-blue-400">
      ...
      {pessoas.length === 0 && (
        <p className="p-20 text-center text-gray-400 italic font-medium">
