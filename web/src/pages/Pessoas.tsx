...
export function Pessoas() {
  ...
  async function salvar(e: React.FormEvent) {
    ...
    try {
      setCarregando(true);
      const dataFormatadaUniversal = new Date(
        dataNascimento + "T00:00:00Z",
      ).toISOString();

      const payload = {
        nome,
        idade: idadeCalculada,
        dataNascimento: dataFormatadaUniversal,
      };
  ...
  async function excluir(id: string, e: React.MouseEvent) {
    ...
    if (
      window.confirm(
        "Tem certeza? Isso apagará todas as transações desta pessoa.",
      )
    ) {
      try {
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="text-left border-l-4 border-blue-600 pl-4">
        ...
        <p className="text-gray-500">
          {editandoId
            ? `Alterando dados de ${nome}`
            : "Gerencie os moradores e seus dados"}
        </p>
      </div>
      ...
      <form
        onSubmit={salvar}
        className={`bg-white p-8 rounded-3xl shadow-xl border-2 transition-all ${editandoId ? "border-orange-400" : "border-transparent"} flex flex-col md:flex-row gap-6 items-end`}
      >
        ...
        <div className="flex gap-2 w-full md:w-auto">
          ...
          <button
            disabled={carregando}
            className={`${editandoId ? "bg-orange-500 hover:bg-orange-600" : "bg-blue-600 hover:bg-blue-700"} text-white font-black py-5 px-10 rounded-2xl shadow-lg transition-all disabled:opacity-50 flex-1`}
          >
        ...
        {currentItems.map((p) => (
          ...
          >
            <div className="text-left">
              <p className="font-black text-gray-800 uppercase text-sm group-hover:text-blue-600 transition-colors">
                {p.nome}
              </p>
            ...
            <div className="flex gap-2">
              <button
                onClick={(e) => prepararEdicao(p, e)}
                className="p-3 rounded-xl bg-orange-50 text-orange-500 opacity-0 group-hover:opacity-100 hover:bg-orange-500 hover:text-white transition-all"
              >
                ✏️
              </button>
              <button
